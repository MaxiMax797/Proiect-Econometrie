...
# Convertește nr_reviewuri la numeric
df$nr_reviewuri <- as.numeric(as.character(df$nr_reviewuri))

# Convertește rating la numeric (dacă există)
if("rating" %in% names(df)) {
  df$rating <- as.numeric(as.character(df$rating))
}

# Tratează valorile NA
df$nr_reviewuri[is.na(df$nr_reviewuri)] <- 0
if("rating" %in% names(df)) {
...
features <- c('Volum', 'Inaltime', 'Alb', 'Argintiu', 'O_usa', 'Doua_usi', 'Minibar',
              ...
              'inaltime_patrat', 'volum_per_usa', 'nr_reviewuri', 'rating')
